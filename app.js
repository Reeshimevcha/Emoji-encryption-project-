/**
 * VoidChat v3.0 — Main Application
 * Features: per-user permissions, session timer, hold-to-send,
 * session persistence, mobile drawer, chat download, VoidChat branding.
 */
...
const S = {
  ...
  canRead:       false,
  canDownload:   false,
  readFrom:      {},     // {userId: bool} — whose msgs I can see
  myVisibleTo:   null,   // null=all, [uids]=private — who sees MY msgs
  roomExpiresAt: null,
  listeners:     [],
  timers:        {},
  schedTimers:   {},
...
function toast(msg, type='info') {
  ...
  const t=ce('div'); t.className=`toast toast-${type}`; t.innerHTML=`<span class="toast-icon">${type==='success'?'✓':type==='error'?'✗':'ℹ'}</span>${msg}`;
  document.body.appendChild(t);
  requestAnimationFrame(()=>t.classList.add('show'));
  setTimeout(()=>{t.classList.remove('show');setTimeout(()=>t.remove(),400);},3500);
}
function copy(text) { navigator.clipboard.writeText(text).then(()=>toast('Copied to clipboard','success')); }
...
async function tryRestoreSession() {
  ...
  try {
    ...
    S.roomCode  = saved.roomCode;
    S.roomType  = saved.roomType;
    S.isOwner   = saved.isOwner;
    S.canRead   = saved.canRead || S.isOwner;
...
async function handleCreate() {
  ...
  S.roomCode=genCode(); S.canRead=true;

  const expiresAt = sessionMin > 0 ? Date.now() + sessionMin*60*1000 : null;
  S.roomExpiresAt = expiresAt;

  await db.ref(`rooms/${S.roomCode}`).set({
    type, maxUsers, owner: S.userId, created: Date.now(),
    expiresAt, closed: false,
  });
  await db.ref(`rooms/${S.roomCode}/users/${S.userId}`).set({
    name, online: true, joinedAt: Date.now(), canRead: true, canDownload: false,
    readFrom: {}, myVisibleTo: null,
  });
  db.ref(`rooms/${S.roomCode}/users/${S.userId}/online`).onDisconnect().set(false);
  saveSession();
  enterChat();
...
async function handleJoin() {
  ...
  S.roomType=room.type; S.canRead=false;
  S.roomExpiresAt=room.expiresAt||null;

  await db.ref(`rooms/${code}/users/${S.userId}`).set({
    name, online: true, joinedAt: Date.now(), canRead: false, canDownload: false,
    readFrom: {}, myVisibleTo: null,
  });
...
function listenRoom() {
  ...
  const uL   = uRef.on('value', snap => {
    ...
    if (me) {
      ...
      S.readFrom     = me.readFrom || {};
      S.myVisibleTo  = me.myVisibleTo || null;
      if (!wasRead && S.canRead) toast('✅ Read access granted by owner','success');
  ...
  root.child('expiresAt').on('value', snap => {
    if (snap.val()) { S.roomExpiresAt=snap.val(); startSessionTimer(); }
  });
...
function startSessionTimer() {
  ...
  S.sessionInterval = setInterval(async () => {
    const rem = S.roomExpiresAt - Date.now();
    if (rem <= 0) {
      clearInterval(S.sessionInterval);
      if (S.isOwner) {
        toast('⏰ Session expired — room purged','info');
        await closeRoom(true);
      } else {
        forceLeave('Session expired');
      }
      return;
    }
    $('session-countdown').textContent = fmtDur(rem);
    if (rem < 300000) $('session-timer-wrap').classList.add('expiring');
  }, 1000);
}
  ...
  for (const [uid,u] of Object.entries(users)) {
    ...
    li.innerHTML=`
      ...
      <div class="um">
        ...
        <span class="us">${u.online?'● ONLINE':'○ OFFLINE'} · ${u.canRead?'🔓 READ':'🔒 ENC'}</span>
      </div>`;
    list.appendChild(li);
  }
...
function renderOwnerPermissions(users) {
    ...
    row.innerHTML=`
      ...
      <div class="pt">
        <div class="toggle-row">
          <span class="tlabel">Read</span>
          <label class="toggle-switch">
            <input type="checkbox" ${u.canRead?'checked':''} onchange="setUserPerm('${uid}','canRead',this.checked)">
            <span class="toggle-track"></span>
          </label>
        </div>
        <div class="toggle-row">
          <span class="tlabel">Download</span>
          <label class="toggle-switch">
            <input type="checkbox" ${u.canDownload?'checked':''} onchange="setUserPerm('${uid}','canDownload',this.checked)">
            <span class="toggle-track"></span>
          </label>
        </div>
      </div>`;
  ...
  if (!hasOthers) panel.innerHTML='<div class="perm-empty">Waiting for users…</div>';
}

async function setUserPerm(uid, field, value) {
  await db.ref(`rooms/${S.roomCode}/users/${uid}/${field}`).set(value);
  toast(`${field==='canRead'?'Read':'Download'} access ${value?'granted':'revoked'}`, value?'success':'info');
}

// ── My Privacy Panel (all users) ───────────────────────────────────────────
function renderMyPrivacy(users) {
  ...
  for (const [uid,u] of Object.entries(users)) {
    ...
    row.innerHTML=`
      ...
      <label class="toggle-switch">
        <input type="checkbox" ${allowed?'checked':''} onchange="toggleMyVisibility('${uid}',this.checked)">
        <span class="toggle-track"></span>
      </label>`;
    panel.appendChild(row);
  }
  if (panel.children.length===0) panel.innerHTML='<div class="perm-empty">No other users yet…</div>';
}
...
async function toggleMyVisibility(uid, allow) {
  ...
  let list = snap.val();
  if (list===null) {
    // Was "all" — build explicit list from all users minus unchecked
  ...
  // If all users included, set to null (public)
  const uSnap=await db.ref(`rooms/${S.roomCode}/users`).once('value');
  ...
  toast(allow?`${uid} can now read your messages`:`${uid} can no longer read your messages`, 'info');
}
...
async function handleSend() {
  ...
  try {
    if (S.scheduledTime) {
      ...
      await schedRef.set({
        from:S.userId, fromName:S.username, timestamp:S.scheduledTime,
        scheduledFor:S.scheduledTime, sent:false, content:enc,
        type:'text', expiresAt:S.destructSecs>0?S.scheduledTime+S.destructSecs*1000:null,
        visibleTo:S.myVisibleTo,
      });
      toast(`⏰ Scheduled for ${fmtDT(S.scheduledTime)}`,'success');
      clearSendOptions();
    } else {
      const msgRef = db.ref(`rooms/${S.roomCode}/messages`).push();
      const msgId  = msgRef.key;
      let data = {
        from:S.userId, fromName:S.username, timestamp:Date.now(),
        expiresAt:S.destructSecs>0?Date.now()+S.destructSecs*1000:null,
        readBy:{[S.userId]:Date.now()}, delivered:{[S.userId]:Date.now()},
        type:'text', visibleTo:S.myVisibleTo,
      };
      if (file) {
...
function scheduleDelivery(schedId, data) {
  ...
  S.schedTimers[schedId] = setTimeout(async () => {
    ...
    await msgRef.set({
      ...
      expiresAt:data.expiresAt, readBy:{[S.userId]:Date.now()},
      delivered:{[S.userId]:Date.now()}, type:data.type||'text',
      content, wasScheduled:true, visibleTo:data.visibleTo||null,
    });
...
function renderMessage(id, data) {
  ...
  if (data.type==='file') {
    ...
    bodyHTML=`<div class="msg-file">
      ...
      <div class="file-row">
        <span class="fib">${getFileIcon(data.fileType)}</span>
        <div class="fim"><span class="fin">${data.fileName}</span><span class="fis">${fmtSz(data.fileSize||0)}</span></div>
        <a class="fdl" href="${data.fileUrl}" download="${data.fileName}" target="_blank">⬇</a>
      </div>
    </div>`;
  }
  ...
  if (data.content) {
    if (canSee) {
      bodyHTML+=`
        <div class="msg-content" id="mc-${id}" data-id="${id}" data-raw="${encodeURIComponent(data.content)}">${data.content}</div>
  ...
  const rxHTML=`<div class="reactions" id="rx-${id}"></div>`;
  ...
  wrap.innerHTML=`
    ...
    <div class="mc">
      ...
      <div class="mb">
        ${timerBar}
        ${data.wasScheduled?'<span class="sched-tag">⏰ SCHEDULED</span>':''}
        ${bodyHTML}
        <div class="mf">
      ...
      ${rxHTML}
      <div class="mha">
        <button class="btn-react" onclick="openReactionPicker('${id}',this)">+</button>
      </div>
    </div>`;
...
function canViewMessage(data) {
  if (S.isOwner && (data.visibleTo===null||data.visibleTo===undefined)) return S.canRead;
  if (data.visibleTo && !data.visibleTo.includes(S.userId)) return false;
  if (!S.canRead) return false;
  if (data.from && S.readFrom && S.readFrom[data.from]===false) return false;
  return true;
}
...
async function decryptAndShow(id, raw) {
  ...
  el.textContent=plain; el.classList.add('revealed');
  S.revealed.add(id);
  const btn=el.nextElementSibling;
...
function openReactionPicker(msgId,btn) {
  ...
  const p=ce('div'); p.className='reaction-picker'; p.id='rp-active';
  ...
  p.style.cssText=`position:fixed;top:${rect.top-70}px;left:${Math.min(rect.left-60,window.innerWidth-220)}px;z-index:9999`;
...
function updateReactions(id,data) {
  ...
  const rxs=data.reactions||{};
  let html='';
  for (const [e,users] of Object.entries(rxs)) {
    const count=Object.keys(users).length; if(!count) continue;
    const mine=!!users[S.userId];
    const emoji=decodeURIComponent(e);
    html+=`<button class="rx-chip ${mine?'mine':''}" onclick="toggleReaction('${id}','${emoji}')">${emoji}<span>${count}</span></button>`;
  }
  el.innerHTML=html;
}
...
// ── Hold-to-Send ────────────────────────────────────────────────────────────
function initHoldToSend() {
  const btn=$('btn-send');
  const showOpts=()=>{
    S.holdActive=true;
    $('send-options-popup').classList.remove('hidden');
    btn.classList.add('hold-active');
  };
  btn.addEventListener('pointerdown', e=>{
    S.holdTimer=setTimeout(showOpts, 800);
  });
  btn.addEventListener('pointerup', e=>{
    if(!S.holdActive){clearTimeout(S.holdTimer);handleSend();}
    S.holdActive=false;
    btn.classList.remove('hold-active');
  });
  btn.addEventListener('pointerleave',()=>{clearTimeout(S.holdTimer);S.holdActive=false;btn.classList.remove('hold-active');});

  $('btn-apply-opts').addEventListener('click',()=>{
    const dt=$('sched-dt').value;
    const ds=parseInt($('destruct-opts').value)||0;
    if(dt){ const ts=new Date(dt).getTime(); if(ts>Date.now()){S.scheduledTime=ts; $('sched-indicator').textContent=`⏰ ${fmtDT(ts)}`; $('sched-indicator').classList.remove('hidden');}}
    S.destructSecs=ds;
    if(ds>0){$('destruct-indicator').textContent=`💣 ${ds<60?ds+'s':ds<3600?ds/60+'m':ds/3600+'h'}`; $('destruct-indicator').classList.remove('hidden');}
    $('send-options-popup').classList.add('hidden');
    toast('Send options set','success');
  });
  $('btn-cancel-opts').addEventListener('click',()=>{$('send-options-popup').classList.add('hidden');});
  $('btn-clear-opts').addEventListener('click',clearSendOptions);
}

function clearSendOptions() {
  S.scheduledTime=null; S.destructSecs=0;
  $('sched-indicator').classList.add('hidden');
  $('destruct-indicator').classList.add('hidden');
  $('send-options-popup').classList.add('hidden');
}
...
<title>VoidChat — Room ${S.roomCode}</title>
<style>
body{font-family:monospace;background:#000;color:#00ff41;padding:20px;}
h1{color:#00d4ff;border-bottom:1px solid #00d4ff;padding-bottom:10px;}
.meta{color:#666;font-size:12px;margin-bottom:20px;}
table{width:100%;border-collapse:collapse;}
th{background:#001a00;color:#00ff41;padding:8px;text-align:left;border-bottom:2px solid #00ff41;}
td{padding:8px;border-bottom:1px solid #003300;vertical-align:top;}
td:first-child{color:#666;white-space:nowrap;width:80px;}
td:nth-child(2){color:#00d4ff;width:100px;font-weight:bold;}
td:last-child{color:#c0ffc0;}
@media print{body{background:white;color:black;}h1,th{color:black;}td{color:#333;}}
</style></head><body>
<h1>🔐 VOIDCHAT — SECURE TRANSCRIPT</h1>
<div class="meta">Room: ${S.roomCode} · Downloaded: ${new Date().toLocaleString()} · ${msgs.length} messages</div>
<table><thead><tr><th>TIME</th><th>FROM</th><th>MESSAGE</th></tr></thead><tbody>${rows}</tbody></table>
</body></html>`;
...
async function closeRoom(auto=false) {
  ...
  if(!auto && !confirm('Close room and permanently delete all messages?')) return;
